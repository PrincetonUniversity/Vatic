...
class VirtualPTDFMatrix(_PTDFManagerBase):
    ...
    def _calculate_PFV(self, mb, masked):
        NWV = np.fromiter((mb._p_nw_tm[b].x for b in self.buses_keys_no_ref),
    ...
    def calculate_LMP(self, LMPC_Constr, LMPI_Constr, LMPE_Constr):
        ...
        if self.contingencies:
            ...
            for (cn,
                 bn), constr in mb.ineq_pf_contingency_branch_thermal_bounds.items():
...
class PTDFMatrix(_PTDFManagerBase):
    ...
    def _calculate_phi_from_phi_to(self):
        return tx_calc.calculate_phi_constant(self._branches,
                                              self.branches_keys,
                                              self.buses_keys,
                                              ApproximationType.PTDF,
                                              mapping_bus_to_idx=self._busname_to_index_map)
    ...
    def _calculate_phase_shift(self):
        ...
        phase_shift_array = np.fromiter((-(1 / branch['reactance']) * (
                    radians(branch['transformer_phase_shift']) / branch[
                'transformer_tap_ratio']) if (
                    branch['branch_type'] == 'transformer') else 0. for branch
                                         in (self._branches[bn] for bn in
                                             self.branches_keys)), float,
                                        count=len(self.branches_keys))
    ...
    def get_branch_const(self, branch_name):
        ...
        PTDF_row = self.PTDFM[row_idx]
        ## phi adj                        +     phase shift
        return PTDF_row.dot(self.phi_adjust_array) + self.phase_shift_array[
            row_idx]
    ...
    def calculate_PFV(self, mb):
        NWV = np.fromiter((value(mb.p_nw[b]) for b in self.buses_keys), float,
...
class PTDFLossesMatrix(PTDFMatrix):
    ...
    def _calculate_ptdf(self):
        ptdf_r, ldf, ldf_c = tx_calc.calculate_ptdf_ldf(self._branches,
                                                        self._buses,
                                                        self.branches_keys,
                                                        self.buses_keys,
                                                        self._reference_bus,
    ...
    def _calculate_phase_shift(self):
        phase_shift_array = np.fromiter((tx_calc.calculate_susceptance(
            branch) * (radians(branch['transformer_phase_shift']) / branch[
            'transformer_tap_ratio'])
                                         if (
                    branch['branch_type'] == 'transformer')
                                         else 0.
                                         for branch in
                                         (self._branches[bn] for bn in
                                          self.branches_keys)), float,
                                        count=len(self.branches_keys))
    ...
    def _calculate_losses_phase_shift(self):
        losses_phase_shift_array = np.fromiter(((tx_calc.calculate_conductance(
            branch) / branch['transformer_tap_ratio']) * radians(
            branch['transformer_phase_shift']) ** 2
                                                if branch[
