"""
Add the total cost objective
"""

from gurobipy import tupledict, LinExpr, quicksum, GRB, Model


def basic_objective(model: Model) -> Model:
    """Calculate the total cost objective"""

    # constraints for computing cost components #
    def compute_no_load_cost_rule(m, g, t):
        return (
                m._MinimumProductionCost[g, t]
                * m._UnitOn[g, t]
                * m._TimePeriodLengthHours
        )

    model._NoLoadCost = {
        (g, t): compute_no_load_cost_rule(model, g, t)
    ...
    def compute_total_production_cost_rule(m, t):
        return sum(
            m._ProductionCost[g, t] for g in m._SingleFuelGenerators
        ) + sum(m._DualFuelProductionCost[g, t] for g in m._DualFuelGenerators)

    model._TotalProductionCost = {
        t: compute_total_production_cost_rule(model, t)
        for t in model._TimePeriods
    }
    ...
    def commitment_stage_cost_expression_rule(m, st):
        cc = sum(
            sum(
                m._NoLoadCost[g, t] + m._StartupCost[g, t]
                for g in m._SingleFuelGenerators
            )
            + sum(
                m._DualFuelCommitmentCost[g, t] for g in m._DualFuelGenerators
            )
        ...
        if m._regulation_service:
            cc += sum(
                m._RegulationCostCommitment[g, t]
                for g in m._AGC_Generators
                for t in m._CommitmentTimeInStage[st]
            )
        return cc

    model._CommitmentStageCost = {
        st: commitment_stage_cost_expression_rule(model, st)
        for st in model._StageSet
    }

    def compute_reserve_shortfall_cost_rule(m, t):
        return (
                m._ReserveShortfallPenalty
                * m._TimePeriodLengthHours
                * m._ReserveShortfall[t]
        )

    model._ReserveShortfallCost = {
        t: compute_reserve_shortfall_cost_rule(model, t)
        for t in model._TimePeriods
    }
    ...
    def generation_stage_cost_expression_rule(m, st):
        cc = (
                sum(
                    sum(m._ProductionCost[g, t] for g in
                        m._SingleFuelGenerators)
                    + sum(
                        m._DualFuelProductionCost[g, t]
                        for g in m._DualFuelGenerators
                    )
                    for t in m._GenerationTimeInStage[st]
                )
                + sum(
            m._LoadMismatchCost[t] for t in m._GenerationTimeInStage[st])
                + sum(
            m._ReserveShortfallCost[t]
            for t in m._GenerationTimeInStage[st]
        )
                + sum(
            m._BranchViolationCost[t] for t in m._GenerationTimeInStage[st]
        )
                + sum(
            m._InterfaceViolationCost[t]
            for t in m._GenerationTimeInStage[st]
        )
                + sum(
            m._ContingencyViolationCost[t]
            for t in m._GenerationTimeInStage[st]
        )
                + sum(
            m._StorageCost[s, t]
        ...
        if m._reactive_power:
            cc += sum(
                m._LoadMismatchCostReactive[t]
                for t in m._GenerationTimeInStage[st]
            )
        if m._security_constraints:
            cc += sum(
                m._SecurityConstraintViolationCost[t]
                for t in m._GenerationTimeInStage[st]
            )
        if m._regulation_service:
            cc += sum(
                m._RegulationCostGeneration[g, t]
                for g in m._AGC_Generators
                for t in m._GenerationTimeInStage[st]
            ) + sum(
                m._RegulationCostPenalty[t]
                for t in m._GenerationTimeInStage[st]
            )
        if m._spinning_reserve:
            cc += sum(
                m._SpinningReserveCostGeneration[g, t]
                for g in m._ThermalGenerators
                for t in m._GenerationTimeInStage[st]
            ) + sum(
                m._SpinningReserveCostPenalty[t]
                for t in m._GenerationTimeInStage[st]
            )
        if m._non_spinning_reserve:
            cc += sum(
                m._NonSpinningReserveCostGeneration[g, t]
                for g in m._NonSpinGenerators
                for t in m._GenerationTimeInStage[st]
            ) + sum(
                m._NonSpinningReserveCostPenalty[t]
                for t in m._GenerationTimeInStage[st]
            )
        if m._supplemental_reserve:
            cc += sum(
                m._SupplementalReserveCostGeneration[g, t]
                for g in m._ThermalGenerators
                for t in m._GenerationTimeInStage[st]
            ) + sum(
                m._SupplementalReserveCostPenalty[t]
                for t in m._GenerationTimeInStage[st]
            )
        if m._flexible_ramping:
            cc += sum(
                m._FlexibleRampingCostPenalty[t]
                for t in m._GenerationTimeInStage[st]
            )
        return cc

    model._GenerationStageCost = {
        st: generation_stage_cost_expression_rule(model, st)
        for st in model._StageSet
    }
    ...
    model._StageCost = {
        st: stage_cost_expression_rule(model, st) for st in model._StageSet
    }
...
def _1bin_shutdown_costs(model, add_shutdown_cost_var=True):
    """Compute shutdown cost under 1 bin variables"""
...
def _3bin_shutdown_costs(model, add_shutdown_cost_var=True):
    """Compute shutdown costs under 3bin variables"""
    ...
    def compute_shutdown_costs_rule(m, g, t):
        ...
        linear_coefs = [-1.0, m._ShutdownFixedCost[g]]
...
def _add_shutdown_costs(model, add_shutdown_cost_var=True):
    """Decide the computation of shutdown cost by checking decision variables"""
    if model._status_vars in [
        "garver_3bin_vars",
    ...
    elif model._status_vars in [
        "CA_1bin_vars",
    ]:
        _1bin_shutdown_costs(model, add_shutdown_cost_var)
    else:
        raise Exception(
            "Problem adding shutdown costs, cannot identify status_vars for this model"
        )
