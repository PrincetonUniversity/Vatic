"""
Add production cost constraints, piecewise production variables, and
...
def KOW_production_costs_tightened(model):
    """
    Add production costs constraints based on
    ...
    33(4), 2018. Equations (19d)--(19h) with some tightening for when SU != SD
    """
...
def _KOW_production_costs(
        model: Model, tightened: Optional[bool] = False
) -> Model:
    """
    ...
    def piecewise_production_limits_rule(m, g, t, i):
        ...
        if t < m._NumTimePeriods:
            ...
            if UT > 1:
                linear_vars = [
                    m._PiecewiseProduction[g, t, i],
                    m._UnitOn[g, t],
                    m._UnitStart[g, t],
                    m._UnitStop[g, t + 1],
                ]
                linear_coefs = [-1.0, (upper - lower), -su_step, -sd_step]
    ...
    model._PiecewiseProductionLimits = model.addConstrs(
        (
            piecewise_production_limits_rule(
                model, g_t_i[0], g_t_i[1], g_t_i[2]
            )
    ...
    model._PiecewiseProductionLimits2 = model.addConstrs(
        (
            piecewise_production_limits_rule2_cons[g_t_i]
...
def CA_production_costs(model: Model) -> Model:
    """
    Add production costs constraints with additional variables for each
    piecewise linear segment based on equations (7)--(11) in
...
def _production_cost_function(m, g, t, i):
    return (
            ...
            * m._PowerGenerationPiecewiseCostValues[g, t][i]
    )
...
def _compute_total_production_cost(model):
    ...
    def compute_production_costs_rule(m, g, t, avg_power):
        ...
        return sum(
            (
                    _production_cost_function(m, g, t, l + 1)
                    - _production_cost_function(m, g, t, l)
            )
            / (piecewise_points[l + 1] - piecewise_points[l])
            * piecewise_eval[l]
            for l in range(len(piecewise_eval))
...
def _get_piecewise_production_generators(model):
    ...
    model._PiecewiseGeneratorTimeIndexSet = list(
        piecewise_generators_time_set(model)
    )
    ...
    model._LinearGeneratorTimeIndexSet = list(
        linear_generators_time_set(model)
    )
    ...
    def piecewise_production_costs_index_set_generator(m):
        return (
            (g, t, i)
            for g, t in m._PiecewiseGeneratorTimeIndexSet
            for i in range(len(m._PowerGenerationPiecewisePoints[g, t]) - 1)
        )

    model._PiecewiseProductionCostsIndexSet = list(
        piecewise_production_costs_index_set_generator(model)
    )
...
def _basic_production_costs_vars(model):
    ...
    model._PiecewiseProduction = model.addVars(
        ...
        lb=0,
        ub=[
            model._PowerGenerationPiecewisePoints[g_t_i[0], g_t_i[1]][
                g_t_i[2] + 1
                ]
            - model._PowerGenerationPiecewisePoints[g_t_i[0], g_t_i[1]][
                g_t_i[2]
...
def _basic_production_costs_constr(model):
    ...
    def piecewise_production_costs_rule(m, g, t):
        if (g, t) in m._PiecewiseGeneratorTimeIndexSet:
            ...
            for i in range(len(points) - 1):
                linear_coefs[i] = (
                                          time_scale * costs[
                                      i + 1] - time_scale * costs[i]
                                  ) / (points[i + 1] - points[i])
                linear_vars[i] = m._PiecewiseProduction[g, t, i]
            return (
                    LinExpr(linear_coefs, linear_vars) - m._ProductionCost[
                g, t]
                    == 0
            )
    ...
    for g in model._SingleFuelGenerators:
        for t in model._TimePeriods:
            ...
            model._ProductionCostConstr[(g, t)] = model.addConstr(
                piecewise_production_costs_rule(model, g, t),
                name="ProductionCostConstr[{},{}]".format(g, t),
            )
