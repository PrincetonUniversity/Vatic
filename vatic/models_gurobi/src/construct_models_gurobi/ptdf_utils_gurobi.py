"""
Helper functions and classes that are useful when
...
class VirtualPTDFMatrix(_PTDFManagerBase):
    ...
    of A^T B_d A instead (or A^T J in internal parlance) and
    ...
    def __init__(
        ...
        some useful methods for interfacing with Gurobipy models
        ...
        ptdf_options (dict) :
            ...
            vatic.models_gurobi.src.construct_models_gurobi.lazy_ptdf_utils_part_gurobi.populate_default_ptdf_options)
        ...
        self._branchname_to_index_map = {
            branch_n: i for i, branch_n in enumerate(self.branches_keys)
        }
        self._busname_to_index_map = {
            bus_n: j for j, bus_n in enumerate(self.buses_keys)
        }
        ...
        self.branch_limits_array = np.fromiter(
            (
                branches[branch]["rating_long_term"]
                for branch in self.branches_keys
            ),
            float,
        ...
        self._bus_sensi_buffer = np.empty(
            (1, len(self.buses_keys_no_ref)), dtype=np.float64
        )
    ...
    def _calculate_ptdf_factorization(self):
        ...
        (
            ...
            B_dA_I,
            I,
        ) = tx_calc.calculate_ptdf_factorization(
            self._branches,
            self._buses,
            self.branches_keys,
            self.buses_keys,
    ...
    def _calculate_interface_limits(self):
        ...
        self.interface_max_limits = np.fromiter(
            _interface_limit_iter("maximum_limit", np.inf),
            float,
            count=len(self.interface_keys),
        )
        self.interface_min_limits = np.fromiter(
            _interface_limit_iter("minimum_limit", -np.inf),
            float,
            count=len(self.interface_keys),
        )
    ...
    def _calculate_contingency_limits(self):
        if self.contingencies:
            ...
            self.contingency_limits_array = np.fromiter(
                _contingency_limit_iter(), float, count=len(self.branches_keys)
            )
        else:
            self.contingency_limits_array = np.empty(
                shape=(len(self.branches_keys), 0)
            )  # create an empty array for slicing on
        self.branch_limits_array.flags.writeable = False
    ...
    def _calculate_phase_shift_flow_adjuster(self):
        self.phase_shift_flow_adjuster_array = (
            tx_calc.calculate_phase_shift_flow_adjuster(
                self._branches, self.branches_keys
            )
        )
    ...
    def _get_filtered_lines(self, ptdf_options):
        if ptdf_options["branch_kv_threshold"] is None:
            ...
            self.contingency_limits_array_masked = (
                self.contingency_limits_array
            )
            return
        ...
        self.branchname_to_index_masked_map = {
            bn: i for i, bn in enumerate(self.branches_keys_masked)
        }
    ...
    def _get_contingency_row(self, contingency_name, branch_name):
        if (contingency_name, branch_name) in self._contingency_rows:
            ...
            hatF = cc.U.solve(
                (self.B_dA[branch_idx] @ cc.Pc).toarray(
                    out=self._bus_sensi_buffer
                )[0],
                "T",
            )
    ...
    def get_contingency_branch_ptdf_iterator(
            self, contingency_name, branch_name
    ):
        """
        returns a (bus_name, coefficient) iterator for a given branch_name
        """
        yield from zip(
            self.buses_keys_no_ref,
            self._get_contingency_row(contingency_name, branch_name),
        )

    def get_contingency_branch_ptdf_abs_max(
            self, contingency_name, branch_name
    ):
        """
        returns the maximum of the absolute value for any coefficent
    ...
    def get_interface_const(self, interface_name):
        ...
        return (
                phi_adj[0]
                + self.phase_shift_flow_adjuster_array_interface[i_idx, 0]
        )
    ...
    def get_interface_ptdf_iterator(self, interface_name):
        ...
        """
        yield from zip(
            self.buses_keys_no_ref, self._get_interface_row(interface_name)
        )

    def _insert_reference_bus(self, bus_array, val):
        return np.insert(
            bus_array, self._busname_to_index_map[self._reference_bus], val
        )
    ...
    def _calculate_PFV(self, mb, masked):
        NWV = np.fromiter(
            (mb._p_nw_tm[b].x for b in self.buses_keys_no_ref),
            float,
        ...
        if masked:
            ...
            VA = -self._insert_reference_bus(VA.T[0], 0.0)
    ...
    def calculate_PFV(self, mb):
        ...
        Parameters
        ----------
        mb : Gurobipy Model with p_nw attribute indexed by buses
    ...
    def calculate_LMP(self, LMPC_Constrs, LMPI_Constrs, LMPE_Constrs):
        ...
        Parameters
        ---
        LMPC_Constrs, LMPI_Constrs, LMPE_Constrs: Model Constraints Related to
        ...
        PFD = np.zeros(len(self.branches_keys_masked))

        if LMPC_Constrs:
            if len(LMPC_Constrs[0]) != 0:
                for i, b_n in enumerate(self.branches_keys_masked):
                    if b_n in LMPC_Constrs[0].keys():
                        PFD[i] = (
                                LMPC_Constrs[0][b_n].Pi + LMPC_Constrs[1][
                            b_n].Pi
                        )

        # interface constributes to LMP
        PFID = np.zeros(len(self.interface_keys))

        if LMPI_Constrs:
            if len(LMPI_Constrs[0]) != 0:
                for i, i_n in enumerate(self.interface_keys):
                    if i_n in LMPI_Constrs[0].keys():
                        PFID[i] = (
                                LMPI_Constrs[0][i_n].Pi + LMPI_Constrs[1][
                            i_n].Pi
                        )
        ...
        if self.contingencies:
            ...
            for (
                        cn,
                        bn,
                ), constr in mb.ineq_pf_contingency_branch_thermal_bounds.items():
    ...
    def calculate_monitored_contingency_flows(self, mb):
        NWV = np.fromiter(
            (mb.p_nw[b].X for b in self.buses_keys_no_ref),
            float,
            count=len(self.buses_keys_no_ref),
        )
        NWV += self.phi_adjust_array.T
...
class PTDFMatrix(_PTDFManagerBase):
    ...
    def _calculate_ptdf_interface(self, interfaces):
        ...
        (
            self.PTDFM_I,
            self.PTDFM_I_const,
        ) = tx_calc.calculate_interface_sensitivities(
            interfaces,
            self.interface_keys,
            self.PTDFM,
    ...
    def _calculate_phi_from_phi_to(self):
        return tx_calc.calculate_phi_constant(
            self._branches,
            self.branches_keys,
            self.buses_keys,
            ApproximationType.PTDF,
    ...
    def get_branch_phase_shift(self, branch_name):
        return self.phase_shift_array[
            self._branchname_to_index_map[branch_name]
        ]
    ...
    def get_branch_const(self, branch_name):
        ...
        # phi adj                        +     phase shift
        return (
                PTDF_row.dot(self.phi_adjust_array)
                + self.phase_shift_array[row_idx]
        )

    def get_interface_const(self, interface_name):
        return self.PTDFM_I_const[
            self.interfacename_to_index_map[interface_name]
        ]
    ...
    def calculate_PFV(self, mb):
        NWV = np.fromiter(
            (mb.p_nw[b].X for b in self.buses_keys),
            float,
...
class PTDFLossesMatrix(PTDFMatrix):
    ...
    def _calculate_phi_loss_constant(self):
        phi_loss_from, phi_loss_to = tx_calc.calculate_phi_loss_constant(
            self._branches,
            self.branches_keys,
            self.buses_keys,
            ApproximationType.PTDF_LOSSES,
        ...
        # sum across the rows to get the total impact, and convert
        # to dense for fast operations later
        self.phi_losses_adjust_array = phi_losses_adjust_array.sum(axis=1).T.A[
            0
        ]
    ...
    def _calculate_losses_phase_shift(self):
        losses_phase_shift_array = np.fromiter(
            (
                (
                        tx_calc.calculate_conductance(branch)
                        / branch["transformer_tap_ratio"]
                )
                * radians(branch["transformer_phase_shift"]) ** 2
                if branch["branch_type"] == "transformer"
                else 0.0
                for branch in (self._branches[bn] for bn in self.branches_keys)
            ),
    ...
    def get_branch_losses_phase_shift(self, branch_name):
        return self.losses_phase_shift_array[
            self._branchname_to_index_map[branch_name]
        ]

    def get_bus_phi_losses_adj(self, bus_name):
        return self.phi_losses_adjust_array[
            self._busname_to_index_map[bus_name]
        ]
