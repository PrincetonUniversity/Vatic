"""
Add systemwise power balance variables and constraints to impose the constraints
on supply-demand mismatch at each bus level and impose the power flow constraints
"""

import vatic.models_gurobi.src.construct_models_gurobi.ptdf_utils_gurobi as ptdf_utils
from .power_vars_gurobi import _add_reactive_power_vars
from .generation_limits_gurobi import _add_reactive_limits
from .lazy_ptdf_utils_part_gurobi import (
    add_monitored_flow_tracker,
    add_initial_monitored_branches,
)
import vatic.models_gurobi.src.transmission_gurobi.bus as libbus
...
from gurobipy import tupledict, LinExpr, GRB, quicksum, Model

from typing import Optional


def ptdf_power_flow(model: Model, slacks: Optional[bool] = True) -> Model:
    """Add constraints to impose the upper bounds on supply-demand mismatch and
    ensure the power flow balance"""
    _add_egret_power_flow(
        model, _ptdf_dcopf_network_model, reactive_power=False, slacks=slacks
    )
    model.update()
    return model


def _add_egret_power_flow(
        model, network_model_builder, reactive_power=False, slacks=True
):
    """Defines generic interface for tramsmission models"""
    ...
    if slacks:
        ...
        if system_load_mismatch:
            raise Exception(
                "_add_blank_system_load_mismatch() is not defined in Egret"
            )
        else:
    ...
    if reactive_power:
        if system_load_mismatch:
            raise Exception(
                "Need to implement system mismatch for reactive power"
            )
    ...
    for tm in model._TimePeriods:
        block._tm = tm
        block._pg = {
            bus: _get_pg_expr_rule(tm, model, bus) for bus in model._Buses
        }
        if reactive_power:
            block._qg = {
                bus: _get_qg_expr_rule(tm, model, bus) for bus in model._Buses
            }
...
def _get_pg_expr_rule(t, m, bus):
    return (
            quicksum(
                m._PowerGeneratedStartupShutdown[g, t]
                for g in m._ThermalGeneratorsAtBus[bus]
            )
            + quicksum(
        m._PowerOutputStorage[s, t] for s in m._StorageAtBus[bus])
            - quicksum(
        m._PowerInputStorage[s, t] for s in m._StorageAtBus[bus])
            + quicksum(m._NondispatchablePowerUsed[g, t]
...
def _add_hvdc(model):
    ...
    model._HVDCLinePower = model.addVars(
        model._HVDCLines,
        model._TimePeriods,
        lb=lbs,
        ub=ubs,
...
def _add_q_load_mismatch(model):
    """Calculate ana add supply and demand mismatch"""
    #
    # load "shedding" can be both positive and negative #
    #
    model._LoadGenerateMismatchReactive = model.addVars(
        model._Buses,
        model._TimePeriods,
        lb=-GRB.INFINITY,
        ub=GRB.INFINITY,
    ...
    model._posLoadGenerateMismatchReactive = model.addVars(
        model._Buses,
        model._TimePeriods,
        lb=0,
        ub=GRB.INFINITY,
        name="posLoadGenerateMismatchReactive",
    )  # load shedding
    model._negLoadGenerateMismatchReactive = model.addVars(
        model._Buses,
        model._TimePeriods,
        lb=0,
        ub=GRB.INFINITY,
        name="negLoadGenerateMismatchReactive",
    )  # over generation
    ...
    model._PosLoadGenerateMismatchToleranceReactive = model.addConstrs(
        (
            pos_load_generate_mismatch_tolerance_rule_reactive(model, b)
            for b in model._Buses
        ),
        name="PosLoadGenerateMismatchToleranceReactive",
    )
    ...
    model._NegLoadGenerateMismatchToleranceReactive = model.addConstrs(
        (
            neg_load_generate_mismatch_tolerance_rule_reactive(model, b)
            for b in model._Buses
        ),
        name="NegLoadGenerateMismatchToleranceReactive",
    )

    def compute_q_load_mismatch_cost_rule(m, t):
        return (
                m._LoadMismatchPenaltyReactive
    ...
    model._LoadMismatchCostReactive = {
        t: compute_q_load_mismatch_cost_rule(model, t)
        for t in model._TimePeriods
    }


def _add_blank_load_mismatch(model):
    model._LoadGenerateMismatch = tupledict(
        {(b, t): 0 for b in model._Buses for t in model._TimePeriods}
    )

    model._posLoadGenerateMismatch = tupledict(
        {(b, t): 0 for b in model._Buses for t in model._TimePeriods}
    )
    model._negLoadGenerateMismatch = tupledict(
        {(b, t): 0 for b in model._Buses for t in model._TimePeriods}
    )
...
def _add_blank_q_load_mismatch(model):
    model._LoadGenerateMismatchReactive = tupledict(
        {(b, t): 0 for b in model._Buses for t in model._TimePeriods}
    )
    model._posLoadGenerateMismatchReactive = tupledict(
        {(b, t): 0 for b in model._Buses for t in model._TimePeriods}
    )
    model._negLoadGenerateMismatchReactive = tupledict(
        {(b, t): 0 for b in model._Buses for t in model._TimePeriods}
    )
...
def _add_system_load_mismatch(model):
    ...
    # load "shedding" can be both positive and negative #
    #
    model._posLoadGenerateMismatch = model.addVars(
        model._TimePeriods,
        lb=0,
        ub=GRB.INFINITY,
    ...
    # load shedding
    model._negLoadGenerateMismatch = model.addVars(
        model._TimePeriods,
        lb=0,
        ub=GRB.INFINITY,
        name="negLoadGenerateMismatch",
    )  # over generation
    ...
    def define_pos_neg_load_generate_mismatch_rule(m, b, t):
        if b == m._ReferenceBus:
            return (
                    m._posLoadGenerateMismatch[t] - m._negLoadGenerateMismatch[
                t]
            )
    ...
    def compute_load_mismatch_cost_rule(m, t):
        linear_vars = [
            *m._posLoadGenerateMismatch.values(),
            *m._negLoadGenerateMismatch.values(),
        ]
        linear_coefs = [
                           m._LoadMismatchPenalty * m._TimePeriodLengthHours
                       ] * len(linear_vars)
        return LinExpr(linear_coefs, linear_vars)

    model._LoadMismatchCost = {
        t: compute_load_mismatch_cost_rule(model, t)
        for t in model._TimePeriods
    }
...
def _add_load_mismatch(model):
    ...
    for b in model._Buses:
        ...
        storage_max_injections = 0.0
        storage_max_withdraws = 0.0
        ...
        for t in model._TimePeriods:
            ...
            if max_injections > 0:
                model._OverGeneration[(b, t)] = model.addVar(
                    lb=0,
                    ub=max_injections,
                    name="OverGeneration[{},{}]".format(b, t),
                )
                over_gen_times_per_bus[b].append(t)
                over_gen_maxes[b, t] = max_injections

            if max_withdrawls > 0:
                model._LoadShedding[(b, t)] = model.addVar(
                    lb=0,
                    ub=max_withdrawls,
                    name="LoadShedding[{},{}]".format(b, t),
                )
...
def _copperplate_network_model(
        block, tm, relax_balance=None, parent_model=None
):
    (
        m,
        gens_by_bus,
...
def _setup_branch_slacks(m, block):
    ...
    block._pf_slack_pos = block.addVars(
        m._BranchesWithSlack, lb=0, ub=GRB.INFINITY, name="pf_slack_pos"
    )

    block._pf_slack_neg = block.addVars(
        m._BranchesWithSlack, lb=0, ub=GRB.INFINITY, name="pf_slack_neg"
    )
...
def _setup_interface_slacks(m, block):
    ...
    # they have a sparse index set
    block._pfi_slack_pos = block.addVars(
        m._InterfacesWithSlack, lb=0, ub=GRB.INFINITY, name="pfi_slack_pos"
    )

    block._pfi_slac_neg = block.addVars(
        m._InterfacesWithSlack, lb=0, ub=GRB.INFINITY, name="pfi_slack_neg"
    )
...
def _setup_contingency_slacks(m, block):
    ...
    block._pfc_slack_pos = block.addVars(
        m._Contingencies,
        m._TransmissionLines,
        lb=0,
        ub=GRB.INFINITY,
        name="pfc_slack_pos",
    )

    block._pfc_slack_neg = block.addVars(
        m._Contingencies,
        m._TransmissionLines,
        lb=0,
        ub=GRB.INFINITY,
...
def _setup_egret_network_topology(m, tm):
    ...
    branches_in_service = tuple(
        l for l in m._TransmissionLines if not m._LineOutOfService[l, tm]
    )
    ...
    branches_out_service = tuple(
        l for l in m._TransmissionLines if m._LineOutOfService[l, tm]
    )
...
def _ptdf_dcopf_network_model(block, tm, parent_model=None):
    ...
    block._p_nw_tm = m.addVars(
        m._Buses, lb=-GRB.INFINITY, ub=GRB.INFINITY, name="p_nw_{}".format(tm)
    )
    ...
    block._contingency_set = [
        (c, i) for c in m._Contingencies for i in m._TransmissionLines
    ]
    ...
    if ptdf_options["lazy"]:
        ...
        libbranch.declare_ineq_p_branch_thermal_bounds(
            ...
            slacks=True,
            slack_cost_expr=m._BranchViolationCost[tm],
        )
        ...
        libbranch.declare_ineq_p_interface_bounds(
            ...
            approximation_type=None,
            slacks=True,
            slack_cost_expr=m._InterfaceViolationCost[tm],
        )
        # declare the "blank" interface flow limits
        libbranch.declare_ineq_p_contingency_branch_thermal_bounds(
            ...
            approximation_type=None,
            slacks=True,
            slack_cost_expr=m._ContingencyViolationCost[tm],
        )

        # add helpers for tracking monitored branches
        add_monitored_flow_tracker(block)
        ...
        p_max = {
            k: branches[k]["rating_long_term"] for k in branches_in_service
        }
