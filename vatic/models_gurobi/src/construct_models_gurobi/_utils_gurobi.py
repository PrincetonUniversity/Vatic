"""
Helper functions that are useful when working with models
"""
...
def _save_uc_results(m: Model, relaxed: bool) -> Model:
    md = m._model_data
    ...
    pg_security_constraints = dict(
        md.elements(
            element_type="security_constraint", security_constraint_type="pg"
        )
    )
    ...
    for constr in ramp_up_avail_potential_constrs:
        ...
        for g, g_dict in thermal_gens.items():
            ...
            for dt, mt in enumerate(m._TimePeriods):
                ...
                if g in m._DualFuelGenerators:
                    ...
                    commitment_cost_dict[dt] += (
                            m._NoLoadCost[g, mt].getValue()
                            + m._StartupCost[g, mt].x
                    )
                ...
                if regulation:
                    if g in m._AGC_Generators:
                        ...
                        commitment_cost_dict[
                            dt
                        ] += m._RegulationCostCommitment[g, mt].x
                        production_cost_dict[
                            dt
                        ] += m._RegulationCostGeneration[g, mt].x
                    else:
                        reg_prov[dt] = 0.0
                        reg_up_supp[dt] = 0.0
                        reg_dn_supp[dt] = 0.0
                ...
                if spin:
                    ...
                    production_cost_dict[
                        dt
                    ] += m._SpinningReserveCostGeneration[g, mt].x

                if nspin:
                    if g in m._NonSpinGenerators:
                        nspin_supp[dt] = m._NonSpinningReserveDispatched[
                            g, mt
                        ].x
                        production_cost_dict[
                            dt
                        ] += m._NonSpinningReserveCostGeneration[g, mt].x
                    else:
                        nspin_supp[dt] = 0.0
                if supp:
                    ...
                    production_cost_dict[
                        dt
                    ] += m._SupplementalReserveCostGeneration[g, mt].x
                if flex:
                ...
                # gurobipy doesn't add constraints that are skiped to the index set, so we also
        ...
        for s, s_dict in storage.items():
            ...
            s_dict["operational_cost"] = _time_series_dict(
        ...
        for sc, sc_dict in pg_security_constraints.items():
            ...
            for dt, mt in enumerate(m._TimePeriods):
                ...
                if sc in b._pgRelaxedSecuritySet:
                    ...
                    sc_violation[dt] = (
                            b._pgSecuritySlackPos[sc].x
                            - b._pgSecuritySlackNeg[sc].x
                    )
        ...
        if m._power_balance == "btheta_power_flow":
                ...
                if l in m._BranchesWithSlack:
                    ...
                    for dt, (mt, b) in enumerate(m._TransmissionBlock.items()):
                        if l in b._pf_slack_pos:
                            pf_violation_dict[dt] = (
                                    b._pf_slack_pos[l].x - b._pf_slack_neg[l].x
                            )
                        else:
                            pf_violation_dict[dt] = 0.0
                    l_dict["pf_violation"] = _time_series_dict(
                        pf_violation_dict
                    )
            ...
            for b, b_dict in buses.items():
                ...
                for dt, mt in enumerate(m._TimePeriods):
                    ...
                    p_balance_violation_dict[dt] = _get_value_general_gurobi(
                        m._LoadGenerateMismatch
                    )
                ...
                if relaxed:
                    ...
                    for dt, mt in enumerate(m._TimePeriods):
                        lmp_dict[dt] = (
                                m._TransmissionBlock[mt]._eq_p_balance[b].Pi
                                / time_period_length_hours
                        )
                ...
                if i in m._InterfacesWithSlack:
                    ...
                    for dt, (mt, b) in enumerate(m._TransmissionBlock.items()):
                        if i in b._pfi_slack_pos:
                            pf_violation_dict[dt] = (
                                    b._pfi_slack_pos[i].x -
                                    b._pfi_slack_neg[i].x
                            )
                        else:
                            pf_violation_dict[dt] = 0.0
                    i_dict["pf_violation"] = _time_series_dict(
                        pf_violation_dict
                    )
            ...
            for mt in m._TimePeriods:
                ...
                if relaxed:
                    ...
                    LMPI_Constrs = None
                    if (
                            hasattr(m, "_ineq_pf_branch_thermal_ub")
                            and len(m._ineq_pf_branch_thermal_ub) != 0
                    ):
                        LMPC_Constrs = [{}, {}]
                        for k in m._ineq_pf_branch_thermal_ub.keys():
                            if k[1] == mt:
                                LMPC_Constrs[0][
                                    k[0]
                                ] = m._ineq_pf_branch_thermal_ub[k]
                                LMPC_Constrs[1][
                                    k[0]
                                ] = m._ineq_pf_branch_thermal_lb[k]

                    if (
                            hasattr(m, "_ineq_pf_interface_ub")
                            and len(m._ineq_pf_interface_ub) != 0
                    ):
                        LMPI_Constrs = [{}, {}]
                        for k in m._ineq_pf_interface_ub.keys():
                            if k[1] == mt:
                                LMPI_Constrs[0][
                                    k[0]
                                ] = m._ineq_pf_interface_ub[k]
                                LMPI_Constrs[1][
                                    k[0]
                                ] = m._ineq_pf_interface_lb[k]

                    LMPE_Constrs = m.getConstrByName(
                        "eq_p_balance_at_period{}".format(mt)
                    )
                    LMP = PTDF.calculate_LMP(
                        LMPC_Constrs, LMPI_Constrs, LMPE_Constrs
                    )
                    lmps_dict[mt] = dict()
                    for i, bn in enumerate(buses_idx):
            ...
            for i, i_dict in interfaces.items():
                ...
                if i in m._InterfacesWithSlack:
                    ...
                    for dt, (mt, b) in enumerate(m._TransmissionBlock.items()):
                        if i in b._pfi_slack_pos:
                            pf_violation_dict[dt] = (
                                    b._pfi_slack_pos[i].x - b._pfi_slack_neg[
                                i].x
                            )
                        else:
                            pf_violation_dict[dt] = 0.0
            ...
            if contingencies:
                for dt, (mt, b) in enumerate(m._TransmissionBlock.items()):
                    contingency_flows = (
                        PTDF.calculate_monitored_contingency_flows(b)
                    )
                    for (cn, bn), flow in contingency_flows.items():
                        ...
                        monitored_branches = c_dict["monitored_branches"][
                            "values"
                        ][dt]
                        monitored_branches[bn] = {"pf": flow}
                        if (cn, bn) in b._pfc_slack_pos:
                            monitored_branches[bn]["pf_violation"] = (
                                    b._pfc_slack_pos[cn, bn].x
                                    - b._pfc_slack_neg[cn, bn].x
                            )
            ...
            for l, l_dict in branches.items():
                ...
                for dt, mt in enumerate(m._TimePeriods):
                    ...
                    pf_dict[dt] = flows_dict[mt].get(l, 0.0)
                l_dict["pf"] = _time_series_dict(pf_dict)
                if l in m._BranchesWithSlack:
                    pf_violation_dict = _preallocated_list(data_time_periods)
                    for dt, (mt, b) in enumerate(m._TransmissionBlock.items()):
            ...
            for b, b_dict in buses.items():
                ...
                if all(x is None for x in va_dict):
                    b_dict["va"] = _time_series_dict(
                        [degrees(v) for v in va_dict]
                    )
                if relaxed:
                    lmp_dict = _preallocated_list(data_time_periods)
                    for dt, mt in enumerate(m._TimePeriods):
                        lmp_dict[dt] = (
                                lmps_dict[mt][b] / time_period_length_hours
                        )
                    b_dict["lmp"] = _time_series_dict(lmp_dict)
            ...
            for l, l_dict in branches.items():
                ...
                if l in m._BranchesWithSlack:
                    ...
                    for dt, mt in enumerate(m._TimePeriods):
                        pf_violation_dict[dt] = (
                                m._BranchSlackPos[l, mt].x
                                - m._BranchSlackNeg[l, mt].x
                        )
                    l_dict["pf_violation"] = _time_series_dict(
                        pf_violation_dict
                    )
            ...
            for i, i_dict in interfaces.items():
                ...
                if i in m._InterfacesWithSlack:
                    ...
                    for dt, mt in enumerate(m._TimePeriods):
                        pf_violation_dict[dt] = (
                                m._InterfaceSlackPos[i, mt].x
                                - m._InterfaceSlackNeg[i, mt].x
                        )
                    i_dict["pf_violation"] = _time_series_dict(
                        pf_violation_dict
                    )
        ...
        elif m._power_balance in [
            "copperplate_power_flow",
            ...
            for dt, mt in enumerate(m._TimePeriods):
                p_viol_dict[dt] = sum(
                    _get_value_general_gurobi(m._LoadGenerateMismatch[b, mt])
                    for b in m._Buses
                )
            ...
            if relaxed:
                ...
                for dt, mt in enumerate(m._TimePeriods):
                    p_price_dict[dt] = (
                            m._TransmissionBlock[mt]._eq_p_balance.Pi
                            / time_period_length_hours
                    )
        ...
        if reserve_requirement:
            ...
            if relaxed:
                ...
                for dt, mt in enumerate(m._TimePeriods):
                    sr_p_dict[dt] = (
                            m._EnforceReserveRequirements[mt].Pi
                            / time_period_length_hours
                    )
        ...
        def _populate_zonal_reserves(elements_dict, string_handle):
            for e, e_dict in elements_dict.items():
                ...
                for req, req_dict in _zonal_reserve_map.items():
                    if req in e_dict:
                        req_shortfall_dict = _preallocated_list(
                            data_time_periods
                        )
                        for dt, mt in enumerate(m._TimePeriods):
                            req_shortfall_dict[dt] = req_dict["shortfall_m"][
                                me, mt
                            ].x
                        ...
                        if relaxed:
                            req_price_dict = _preallocated_list(
                                data_time_periods
                            )
                            for dt, mt in enumerate(m._TimePeriods):
                                req_price_dict[dt] = (
                                        req_dict["balance_m"][me, mt].Pi
                                        / time_period_length_hours
                                )
            ...
            for f, f_dict in fuel_supplies.items():
                ...
                if fuel_supply_type == "instantaneous":
                    for dt, mt in enumerate(m._TimePeriods):
                        fuel_consumed[
                            dt
                        ] = m._TotalFuelConsumedAtInstFuelSupply[f, mt].x
                else:
                    logger.warning(
