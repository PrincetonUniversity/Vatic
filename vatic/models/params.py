...
def default_params(model, model_data):
    ...
    model.TimePeriodLengthMinutes = pe.Param(default=60, within=pe.PositiveIntegers, initialize=system['time_period_length_minutes'])

    ## IN HOURS, assert athat this must be a positive number
    model.TimePeriodLengthHours = pe.Param(default=pe.value(model.TimePeriodLengthMinutes)/60., within=pe.PositiveReals)

    model.NumTimePeriods = pe.Param(within=pe.PositiveIntegers, initialize=len(system['time_keys']))
    ...
    model.CommitmentTimeInStage = pe.Set(model.StageSet, within=model.TimePeriods,
                                        initialize={'Stage_1':model.TimePeriods, 'Stage_2': list() } )
    model.GenerationTimeInStage = pe.Set(model.StageSet, within=model.TimePeriods,
    ...
    model.BusFrom = pe.Param(model.TransmissionLines, within=model.Buses, initialize=branch_attrs.get('from_bus', dict()))
    model.BusTo   = pe.Param(model.TransmissionLines, within=model.Buses, initialize=branch_attrs.get('to_bus', dict()))

    model.HVDCBusFrom = pe.Param(model.HVDCLines, within=model.Buses, initialize=dc_branch_attrs.get('from_bus', dict()))
    model.HVDCBusTo   = pe.Param(model.HVDCLines, within=model.Buses, initialize=dc_branch_attrs.get('to_bus', dict()))
    ...
    model.HVDCLinesTo = pe.Set(model.Buses, initialize=dc_inlet_branches_by_bus)
    model.HVDCLinesFrom = pe.Set(model.Buses, initialize=dc_outlet_branches_by_bus)
    ...
    def _warn_neg_impedence(m, v, l):
        if v == 0.:
            ...
            return True
        return True
    model.Impedence = pe.Param(model.TransmissionLines, within=pe.Reals, initialize=branch_attrs.get('reactance', dict()), validate=_warn_neg_impedence)

    model.ThermalLimit = pe.Param(model.TransmissionLines, initialize=branch_attrs.get('rating_long_term', dict())) # max flow across the line
    model.HVDCThermalLimit = pe.Param(model.HVDCLines, initialize=dc_branch_attrs.get('rating_long_term', dict())) # max flow across the line

    model.LineOutOfService = pe.Param(model.TransmissionLines, model.TimePeriods, within=pe.Boolean, default=False,
                                    initialize=TimeMapper(branch_attrs.get('planned_outage', dict())))

    model.HVDCLineOutOfService = pe.Param(model.HVDCLines, model.TimePeriods, within=pe.Boolean, default=False,
    ...
    model.BranchesWithSlack = pe.Set(within=model.TransmissionLines, initialize=_branch_penalties.keys())

    model.BranchLimitPenalty = pe.Param(model.BranchesWithSlack, within=pe.NonNegativeReals, initialize=_branch_penalties)

    ## Interfaces
    model.Interfaces = pe.Set(initialize=interface_attrs['names'])

    model.InterfaceLines = pe.Set(model.Interfaces, within=model.TransmissionLines, initialize=interface_attrs.get('lines', dict()), ordered=True)
    model.InterfaceMinFlow = pe.Param(model.Interfaces, within=pe.Reals, initialize=interface_attrs.get('minimum_limit', dict()))
    model.InterfaceMaxFlow = pe.Param(model.Interfaces, within=pe.Reals, initialize=interface_attrs.get('maximum_limit', dict()))
    ...
    model.CheckInterfaceFlowLimits = pe.BuildAction(rule=check_min_less_max_interface_flow_limits)

    def get_interface_line_pairs(m):
        for i in m.Interfaces:
            for l in m.InterfaceLines[i]:
                yield i,l
    model.InterfaceLinePairs = pe.Set(initialize=get_interface_line_pairs, dimen=2)
    ...
    model.InterfaceLineOrientation = pe.Param(model.InterfaceLinePairs, initialize=_interface_line_orientation_dict, within=set([-1,0,1]))
    ...
    model.InterfacesWithSlack = pe.Set(within=model.Interfaces, initialize=_interface_penalties.keys())

    model.InterfaceLimitPenalty = pe.Param(model.InterfacesWithSlack, within=pe.NonNegativeReals, initialize=_interface_penalties)
    ...
    model.ThermalGeneratorsAtBus = pe.Set(model.Buses, initialize=thermal_gens_by_bus)

    model.ThermalGeneratorType = pe.Param(model.ThermalGenerators, within=pe.Any, default='C', initialize=thermal_gen_attrs.get('fuel', dict()))
    ...
    assert set(model.ThermalGenerators) == {
        gen for b in model.Buses for gen in model.ThermalGeneratorsAtBus[b]}

    model.QuickStart = pe.Param(model.ThermalGenerators, within=pe.Boolean, default=False, initialize=thermal_gen_attrs.get('fast_start', dict()))

    def init_quick_start_generators(m):
        return [g for g in m.ThermalGenerators
                if pe.value(m.QuickStart[g]) == 1]

    model.QuickStartGenerators = pe.Set(within=model.ThermalGenerators, initialize=init_quick_start_generators)

    # optionally force a unit to be on/off
    model.FixedCommitmentTypes = pe.Set(initialize=[0,1,None])
    model.FixedCommitment = pe.Param(model.ThermalGenerators,
                                  model.TimePeriods,
                                  within=model.FixedCommitmentTypes,
                                  default=None,
                                  initialize=TimeMapper(thermal_gen_attrs.get('fixed_commitment', dict())),)

    model.NondispatchableGeneratorsAtBus = pe.Set(model.Buses, initialize=renewable_gens_by_bus)

    model.AllNondispatchableGenerators = pe.Set(initialize=renewable_gen_attrs['names'])

    model.NondispatchableGeneratorType = pe.Param(model.AllNondispatchableGenerators, within=pe.Any, default='W',
                                                initialize=renewable_gen_attrs.get('fuel', dict()))

    assert set(model.AllNondispatchableGenerators) == {
        gen for b in model.Buses
        for gen in model.NondispatchableGeneratorsAtBus[b]
        }
    ...
    model.Demand = pe.Param(model.Buses, model.TimePeriods, initialize=bus_loads, mutable=True)

    def calculate_total_demand(m, t):
        return sum(pe.value(m.Demand[b,t]) for b in sorted(m.Buses))
    model.TotalDemand = pe.Param(model.TimePeriods, initialize=calculate_total_demand)
    ...
    if warn_neg_load:
        model.WarnAboutNegativeDemand = pe.BuildAction(model.Buses, model.TimePeriods, rule=warn_about_negative_demand_rule)
    ...
    model.ReserveRequirement = pe.Param(model.TimePeriods, within=pe.NonNegativeReals,
    ...
    model.MinimumUpTime = pe.Param(model.ThermalGenerators,
                                    within=pe.NonNegativeReals,
                                    default=0,
                                    initialize=thermal_gen_attrs['min_up_time'])
    model.MinimumDownTime = pe.Param(model.ThermalGenerators,
                                    within=pe.NonNegativeReals,
                                    default=0,
    ...
    model.ScaledMinimumUpTime = pe.Param(model.ThermalGenerators, within=pe.NonNegativeIntegers, initialize=scale_min_uptime)
    ...
    model.ScaledMinimumDownTime = pe.Param(model.ThermalGenerators, within=pe.NonNegativeIntegers, initialize=scale_min_downtime)
    ...
    model.MinimumPowerOutput = pe.Param(model.ThermalGenerators, model.TimePeriods,
                                        within=pe.NonNegativeReals,
    ...
    model.MaximumPowerOutput = pe.Param(model.ThermalGenerators, model.TimePeriods,
                                        within=pe.NonNegativeReals,
                                        validate=maximum_power_output_validator,
                                        initialize=TimeMapper(thermal_gen_attrs['p_max']),
                                        default=0.0)

    model.MinimumReactivePowerOutput = pe.Param(model.ThermalGenerators, model.TimePeriods,
                                                within=pe.Reals,
    ...
    model.MaximumReactivePowerOutput = pe.Param(model.ThermalGenerators, model.TimePeriods,
                                                within=pe.Reals,
    ...
    model.MinNondispatchablePower = pe.Param(model.AllNondispatchableGenerators,
                                            model.TimePeriods,
    ...
    model.MaxNondispatchablePower = pe.Param(model.AllNondispatchableGenerators,
                                            model.TimePeriods,
                                            within=pe.Reals, # more permissive; e.g. CSP
                                            default=0.0,
                                            mutable=True,
    ...
    def ramp_up_validator(m, v, g):
        ...
        for t in m.TimePeriods:
            if t == t1:
                continue
            diff = pe.value(m.MinimumPowerOutput[g, t] - m.MaximumPowerOutput[g, t-1])
    ...
    def ramp_down_validator(m, v, g):
        ...
        for t in m.TimePeriods:
            if t == t1:
                continue
            diff = pe.value(m.MinimumPowerOutput[g, t - 1] - m.MaximumPowerOutput[g, t])
            if v*m.TimePeriodLengthHours < diff:
    ...
    model.NominalRampUpLimit = pe.Param(model.ThermalGenerators,
                                        within=pe.NonNegativeReals,
                                        mutable=True,
                                        initialize=thermal_gen_attrs['ramp_up_60min'],
                                        validate=ramp_up_validator)
    model.NominalRampDownLimit = pe.Param(model.ThermalGenerators,
                                        within=pe.NonNegativeReals,
                                        mutable=True,
    ...
    model.UnitOnT0State = pe.Param(model.ThermalGenerators,
                                within=pe.Reals,
    ...
    model.FutureStatus = pe.Param(model.ThermalGenerators,
                               within=pe.Reals,
                               mutable=True,
                               default=0.,
    ...
    def time_periods_since_last_shutdown_rule(m,g):
        if pe.value(m.UnitOnT0[g]):
            ...
            return int(math.ceil( -pe.value(m.UnitOnT0State[g]) / pe.value(m.TimePeriodLengthHours) ))
    model.TimePeriodsSinceShutdown = pe.Param(model.ThermalGenerators, within=pe.PositiveIntegers, mutable=True,
    ...
    def time_periods_before_startup_rule(m,g):
        if pe.value(m.FutureStatus[g]) <= 0:
            ...
            return int(math.ceil( pe.value(m.FutureStatus[g]) /  pe.value(m.TimePeriodLengthHours) ))
    model.TimePeriodsBeforeStartup = pe.Param(model.ThermalGenerators, within=pe.PositiveIntegers, mutable=True,
    ...
    model.StartupCurve = pe.Set(model.ThermalGenerators, within=pe.NonNegativeReals, ordered=True, initialize=startup_curve_init_rule)
    ...
    model.ShutdownCurve = pe.Set(model.ThermalGenerators, within=pe.NonNegativeReals, ordered=True, initialize=shutdown_curve_init_rule)
    ...
    def power_generated_t0_validator(m, v, g):
        ...
        if pe.value(m.UnitOnT0[g]):
            v_less_max = v <= pe.value(m.MaximumPowerOutput[g,t] + m.NominalRampDownLimit[g]*m.TimePeriodLengthHours)
            if not v_less_max:
                logger.error('Generator {} has more output at T0 than is feasible to ramp down to'.format(g))
                return False
            v_greater_min = v >= pe.value(m.MinimumPowerOutput[g,t] - m.NominalRampUpLimit[g]*m.TimePeriodLengthHours)
            if not v_less_max:
    ...
    model.PowerGeneratedT0 = pe.Param(model.ThermalGenerators,
                                   within=pe.NonNegativeReals,
    ...
    model.StartupRampLimit = pe.Param(model.ThermalGenerators,
                                    model.TimePeriods,
                                    within=pe.NonNegativeReals,
    ...
    model.ShutdownRampLimit = pe.Param(model.ThermalGenerators,
                                    model.TimePeriods,
                                    within=pe.NonNegativeReals,
    ...
    def scale_ramp_up(m, g, t):
        ...
        if pe.value(temp) > pe.value(m.MaximumPowerOutput[g,t]):
            ...
            return temp
    model.ScaledNominalRampUpLimit = pe.Param(model.ThermalGenerators, model.TimePeriods,  within=pe.NonNegativeReals, initialize=scale_ramp_up, mutable=True)

    def scale_ramp_down(m, g, t):
        temp = m.NominalRampDownLimit[g] * m.TimePeriodLengthHours
        if t == m.InitialTime:
            param = max(pe.value(m.PowerGeneratedT0[g]), pe.value(m.MaximumPowerOutput[g,t]))
        else:
        ...
        if pe.value(temp) > pe.value(param):
            ...
            return temp
    model.ScaledNominalRampDownLimit = pe.Param(model.ThermalGenerators, model.TimePeriods, within=pe.NonNegativeReals, initialize=scale_ramp_down, mutable=True)
    ...
    def scale_startup_limit(m, g, t):
        ...
        if pe.value(temp) > pe.value(m.MaximumPowerOutput[g,t] - m.MinimumPowerOutput[g,t]):
            return m.MaximumPowerOutput[g,t]
        else:
            return temp + m.MinimumPowerOutput[g,t]
    model.ScaledStartupRampLimit = pe.Param(model.ThermalGenerators, model.TimePeriods, within=pe.NonNegativeReals, validate=ramp_limit_validator, initialize=scale_startup_limit, mutable=True)
    ...
    def scale_shutdown_limit(m, g, t):
        ...
        if pe.value(temp) > pe.value(m.MaximumPowerOutput[g,t] - m.MinimumPowerOutput[g,t]):
            ...
            return temp + m.MinimumPowerOutput[g,t]
    model.ScaledShutdownRampLimit = pe.Param(model.ThermalGenerators, model.TimePeriods, within=pe.NonNegativeReals, validate=ramp_limit_validator, initialize=scale_shutdown_limit, mutable=True)
    ...
    def scale_shutdown_limit_t0(m, g):
        ...
        if pe.value(temp) > pe.value(m.PowerGeneratedT0[g] - m.MinimumPowerOutputT0[g]):
            return m.PowerGeneratedT0[g]
        else:
            return temp + m.MinimumPowerOutputT0[g]
    model.ScaledShutdownRampLimitT0 = pe.Param(model.ThermalGenerators, within=pe.NonNegativeReals, initialize=scale_shutdown_limit_t0, mutable=True)
    ...
    def startup_lags_init_rule(m, g):
        ...
        if startup_fuel is None and startup_cost is None:
            ...
            return _get_startup_lag(startup_fuel, pe.value(m.MinimumDownTime[g]))
        else:
            return _get_startup_lag(startup_cost, pe.value(m.MinimumDownTime[g]))
    model.StartupLags = pe.Set(model.ThermalGenerators, within=pe.NonNegativeReals, ordered=True, initialize=startup_lags_init_rule) # units are hours / time periods.
    ...
    model.StartupCosts = pe.Set(model.ThermalGenerators, within=pe.NonNegativeReals, ordered=True, initialize=startup_costs_init_rule) # units are $.
    ...
    def validate_startup_lags_rule(m, g):
        ...
        if startup_lags[0] != pe.value(m.MinimumDownTime[g]):
           print("DATA ERROR: The first startup lag for thermal generator="+str(g)+" must be equal the minimum down time="+str(pe.value(m.MinimumDownTime[g]))+".")
           assert(False)
    ...
    model.ValidateStartupLags = pe.BuildAction(model.ThermalGenerators, rule=validate_startup_lags_rule)
    ...
    model.ValidateStartupCosts = pe.BuildAction(model.ThermalGenerators, rule=validate_startup_costs_rule)
    ...
    model.ValidateStartupLagCostCardinalities = pe.BuildAction(model.ThermalGenerators, rule=validate_startup_lag_cost_cardinalities)
    ...
    model.StartupCostIndices = pe.Set(model.ThermalGenerators, within=pe.NonNegativeIntegers, initialize=startup_cost_indices_init_rule)
    ...
    model.ScaledStartupLags = pe.Set(model.ThermalGenerators, within=pe.NonNegativeIntegers, ordered=True, initialize=scaled_startup_lags_rule)

    ##################################################################################
    # shutdown cost for each generator. in the literature, these are often set to 0. #
    ##################################################################################

    model.ShutdownFixedCost = pe.Param(model.ThermalGenerators, within=pe.NonNegativeReals, default=0.0, initialize=thermal_gen_attrs.get('shutdown_cost', dict())) # units are $.
    ...
    model.FuelSupplyGenerators = pe.Set(within=model.ThermalGenerators, initialize=fuel_supply_gens_init, validate=gen_cost_fuel_validator)
    ...
    def dual_fuel_init(m):
        for g, g_dict in thermal_gens.items():
            if 'aux_fuel_capable' in g_dict and g_dict['aux_fuel_capable']:
                yield g
    model.DualFuelGenerators = pe.Set(within=model.ThermalGenerators, initialize=dual_fuel_init)
    ...
    model.ValidateGeneratorCost = pe.BuildCheck(model.ThermalGenerators, rule=validate_cost_rule)
    ...
    model.NumGeneratorCostCurvePieces = pe.Param(within=pe.PositiveIntegers, default=2, mutable=True)
    ...
    model.CreatePowerGenerationPiecewisePoints = pe.BuildAction(model.ThermalGenerators, rule=power_generation_piecewise_points_rule)
    ...
    model.MinimumProductionCost = pe.Param(model.SingleFuelGenerators, model.TimePeriods, within=pe.NonNegativeReals, initialize=_minimum_production_cost, mutable=True)

    model.MinimumFuelConsumption = pe.Param(model.FuelSupplyGenerators, model.TimePeriods, within=pe.NonNegativeReals, initialize=_minimum_fuel_consumption, mutable=True)
    ...
    model.ReserveShortfallPenalty = pe.Param(within=pe.NonNegativeReals, default=ModeratelyBigPenalty, mutable=True, initialize=system.get('reserve_shortfall_cost', ModeratelyBigPenalty))

    BigPenalty = 1e4*system['baseMVA']

    model.LoadMismatchPenalty = pe.Param(within=pe.NonNegativeReals, mutable=True, initialize=system.get('load_mismatch_cost', BigPenalty))
    model.LoadMismatchPenaltyReactive = pe.Param(within=pe.NonNegativeReals, mutable=True, initialize=system.get('q_load_mismatch_cost', BigPenalty/2.))

    model.Contingencies = pe.Set(initialize=contingencies.keys())

    # leaving this unindexed for now for simpility
    model.ContingencyLimitPenalty = pe.Param(within=pe.NonNegativeReals, initialize=system.get('contingency_flow_violation_cost', BigPenalty/2.), mutable=True)
    ...
    model.VerifyStorageBuses = pe.BuildAction(model.Storage, rule=verify_storage_buses_rule)
    ...
    model.MinimumPowerOutputStorage = pe.Param(model.Storage, within=pe.NonNegativeReals,
    ...
    model.MaximumPowerOutputStorage = pe.Param(model.Storage, within=pe.NonNegativeReals,
    ...
    model.MinimumPowerInputStorage = pe.Param(model.Storage, within=pe.NonNegativeReals,
    ...
    model.MaximumPowerInputStorage = pe.Param(model.Storage, within=pe.NonNegativeReals,
    ...
    model.NominalRampUpLimitStorageOutput    = pe.Param(model.Storage, within=pe.NonNegativeReals,
                                                        initialize=storage_attrs.get('ramp_up_output_60min', dict()))
    model.NominalRampDownLimitStorageOutput  = pe.Param(model.Storage, within=pe.NonNegativeReals,
                                                        initialize=storage_attrs.get('ramp_down_output_60min', dict()))

    # ramp rate limits when charging
    model.NominalRampUpLimitStorageInput     = pe.Param(model.Storage, within=pe.NonNegativeReals,
                                                        initialize=storage_attrs.get('ramp_up_input_60min', dict()))
    model.NominalRampDownLimitStorageInput   = pe.Param(model.Storage, within=pe.NonNegativeReals,
    ...
    model.ScaledNominalRampUpLimitStorageOutput = pe.Param(model.Storage, within=pe.NonNegativeReals, initialize=scale_storage_ramp_up_out)

    def scale_storage_ramp_down_out(m, s):
        return m.NominalRampDownLimitStorageOutput[s] * m.TimePeriodLengthHours
    model.ScaledNominalRampDownLimitStorageOutput = pe.Param(model.Storage, within=pe.NonNegativeReals, initialize=scale_storage_ramp_down_out)

    def scale_storage_ramp_up_in(m, s):
        return m.NominalRampUpLimitStorageInput[s] * m.TimePeriodLengthHours
    model.ScaledNominalRampUpLimitStorageInput = pe.Param(model.Storage, within=pe.NonNegativeReals, initialize=scale_storage_ramp_up_in)

    def scale_storage_ramp_down_in(m, s):
        return m.NominalRampDownLimitStorageInput[s] * m.TimePeriodLengthHours
    model.ScaledNominalRampDownLimitStorageInput = pe.Param(model.Storage, within=pe.NonNegativeReals, initialize=scale_storage_ramp_down_in)
    ...
    model.MaximumEnergyStorage = pe.Param(model.Storage, within=pe.NonNegativeReals, default=0.0,
                                        initialize=storage_attrs.get('energy_capacity', dict()))
    model.MinimumSocStorage = pe.Param(model.Storage, within=pe.PercentFraction, default=0.0,
    ...
    model.InputEfficiencyEnergy  = pe.Param(model.Storage, within=pe.PercentFraction, default=1.0,
                                            initialize=storage_attrs.get('charge_efficiency', dict()))
    model.OutputEfficiencyEnergy = pe.Param(model.Storage, within=pe.PercentFraction, default=1.0,
                                            initialize=storage_attrs.get('discharge_efficienty', dict()))
    model.RetentionRate          = pe.Param(model.Storage, within=pe.PercentFraction, default=1.0,
                                            initialize=storage_attrs.get('retention_rate_60min', dict())) ## assumed to be %/hr

    model.ChargeCost = pe.Param(model.Storage, within=pe.Reals, default=0.0, initialize=storage_attrs.get('charge_cost', dict()))
    model.DischargeCost = pe.Param(model.Storage, within=pe.Reals, default=0.0, initialize=storage_attrs.get('discharge_cost', dict()))

    ## this will be multiplied by itself 1/m.TimePeriodLengthHours times, so this is the scaling to
    ## get us back to %/hr
    def scaled_retention_rate(m,s):
        return pe.value(m.RetentionRate[s])** pe.value(m.TimePeriodLengthHours)
    model.ScaledRetentionRate = pe.Param(model.Storage, within=pe.PercentFraction, initialize=scaled_retention_rate)
    ...
    model.EndPointSocStorage = pe.Param(model.Storage, within=pe.PercentFraction, default=0.5,
    ...
    def t0_storage_power_input_validator(m, v, s):
        return (v >= pe.value(m.MinimumPowerInputStorage[s])) and (v <= pe.value(m.MaximumPowerInputStorage[s]))

    def t0_storage_power_output_validator(m, v, s):
        return (v >= pe.value(m.MinimumPowerInputStorage[s])) and (v <= pe.value(m.MaximumPowerInputStorage[s]))

    model.StoragePowerOutputOnT0 = pe.Param(model.Storage, within=pe.NonNegativeReals,
                                            validate=t0_storage_power_output_validator,
                                            default=0.0,
                                            initialize=storage_attrs.get('initial_discharge_rate', dict()))
    model.StoragePowerInputOnT0  = pe.Param(model.Storage, within=pe.NonNegativeReals,
                                            validate=t0_storage_power_input_validator,
                                            default=0.0,
                                            initialize=storage_attrs.get('initial_charge_rate', dict()))
    model.StorageSocOnT0         = pe.Param(model.Storage, within=pe.PercentFraction,
...
def renew_cost_params(model: pe.ConcreteModel,
    ...
    model.BusFrom = pe.Param(model.TransmissionLines,
                             within=model.Buses,
                             initialize=branch_attrs.get('from_bus', dict()))
    model.BusTo   = pe.Param(model.TransmissionLines,
                             within=model.Buses,
    ...
    model.Impedence = pe.Param(
        ...
        within=pe.Reals, initialize=branch_attrs.get('reactance', dict()),
    ...
    model.HVDCLineOutOfService = pe.Param(
        model.HVDCLines, model.TimePeriods,
        within=pe.Boolean, default=False,
        initialize=TimeMapper(dc_branch_attrs.get('planned_outage', dict()))
        )
    ...
    model.InterfaceLines = pe.Set(
        model.Interfaces,
        within=model.TransmissionLines,
    ...
    def verify_thermal_generator_buses_rule(m, gen):
       for b in m.Buses:
          if gen in m.ThermalGeneratorsAtBus[b]:
             return

       raise ModelError("DATA ERROR: No bus assigned for thermal generator "
                        "`{}`!".format(gen))

    model.VerifyThermalGeneratorBuses = pe.BuildAction(
        model.ThermalGenerators, rule=verify_thermal_generator_buses_rule)
    ...
    model.MinimumPowerOutput = pe.Param(
        model.ThermalGenerators | model.AllNondispatchableGenerators,
        model.TimePeriods,
        within=pe.NonNegativeReals,
        initialize=TimeMapper(gen_attrs['p_min']), default=0.0
        )
    ...
    model.MaximumPowerOutput = pe.Param(
        ...
        within=pe.NonNegativeReals, default=0.,
        validate=maximum_power_output_validator,
        initialize=TimeMapper(gen_attrs['p_max']),
        )
    ...
    model.CreatePowerGenerationPiecewisePoints = pe.BuildAction(
        model.ThermalGenerators | model.AllNondispatchableGenerators,
        rule=power_generation_piecewise_points_rule
        )
